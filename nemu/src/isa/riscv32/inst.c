...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = imm + s->pc; R(rd) = s->pc + 4;
		  //for ftrace 
		  IFDEF(CONFIG_ITRACE,if(rd==1) {ftrace_call(s->pc,s->dnpc,false);}));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (imm + src1) & (~1) ; R(rd) = s->pc + 4;
		  //for ftrace
		  IFDEF(CONFIG_ITRACE,if(rd==1) {ftrace_call(s->pc,s->dnpc,false);}
			              else if(s->isa.inst.val == 0x00008067) { ftrace_ret(s->pc);}
                                      else if( rd==0 && imm ==0) {ftrace_call(s->pc,s->dnpc,true);}));
